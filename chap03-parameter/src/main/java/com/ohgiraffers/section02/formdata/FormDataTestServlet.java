...
public class FormDataTestServlet extends HttpServlet {
    ...
    * 변환 후 url 요청으로 전송한다. 이 때 header의 내용은 ascii 코드로 전송되기 때문에 어떤 언어든 서버의 설정 인코딩 방식과
    ...
    * 해더와는 별개로 urlEncoder를 이용하지 않고 페이지 meta에 기술된 charset에 따라 UTF-8로 해석된 데이터를 서버로 전송한다.
    * 기본적으로 서버단에서 페이로드를 디코딩 하는 방식은 지정되어 있지 않으므로 request, getCharacterEncoding()을 호출해보면
    ...
    * setCharacterEncoding(String encType)으로 지정해주어야 브라우저에서 요청한 인코딩 방식과 일치하여 한글 꺠짐을 막을 수 있다.
    * */
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

        // 기본설정 인코딩 방식이 null이므로 톰캣의 기본 세팅값이 iso-8859-1로 디코딩을 시도한다.
