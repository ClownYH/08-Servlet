...
public class PrintLoginSuccessServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        /*
        * forward 받은 서블릿에서도 요청 방식이 get이면 doget 메소드를, 요청 방식이 post면 doPost 메소드를 호출한다.
        ...
        * 하지만 forward 방식은 response를 포함하여 위임하기 때문에 request에 정보를 저장하여 forward하면
        * 위임받은 서블릿에서도 위임한 서블릿의 정보를 공유할 수 있다.
        * forward 받은 서블릿의 존재를 클라이언트가 알 필요가 없기 때문에 url 자체는 변경되지 않는다.
        * (사용자는 결과화면만 제대로 받으면 되기 때문이다.)
        * forward 방식의 또 다른 특징은 요청 시 서버로 전속한 데이터가 남아있는 상태로 새로고침(=재요청)을 하면
        * 동일한 요청을 반복하여 데이터베이스에 insert하는 등의 행위를 하면 중볻된 행이 삽입될 가능성이 있다.
