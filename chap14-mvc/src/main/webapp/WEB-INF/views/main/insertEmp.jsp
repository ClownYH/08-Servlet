...
<body>
  ...
  <h3>emp 테이블에 신규 직원 정보 추가(필요한 정보를 입력받을 수 있는 form을 만들어서 추가함)</h3>
  <form action="${pageContext.servletContext.contextPath}/employees" method="post">
    ...
    이메일 : <input type="email" name="email"/> (입력하지 않아도 괜찮습니다.) <br>
    전화번호(- 제외) : <input type="tel" name="phone"/> (입력하지 않아도 괜찮습니다.) <br>
    부서코드 :
...
<h3>emp 테이블 정보 수정 요청</h3>
<form action="${pageContext.servletContext.contextPath}/employees2" method="post">
  직원 번호 : <input type="number" name="empId"/><br>
...
<h3>직원 정보 삭제 요청</h3>
  <form action="${pageContext.servletContext.contextPath}/employees3" method="post">
    삭제하고자 하는 직원의 번호를 입력해주세요 : <br>
